...
describe('WalletCrypto', () => {
  ...
  describe('decryptPasswordWithProcessedPin()', () =>
    it('should return the password', () => {
      let data = 'pKPZ1y8lM9aixrcaW3VGHdZkQL37ESyFQvgxVSIjhEo=';
      let password = '60ae40d723196edea0ae35ace25db8961905dd8582ae813801c7494e71173925';
      let pbkdf2Iterations = 1;

      let decryptedPassword = WalletCrypto.decryptPasswordWithProcessedPin(data, password, pbkdf2Iterations);

      expect(decryptedPassword).toBe('testtest12');
    })
  );
  ...
  describe('decrypt', () => {
    ...
    beforeEach(() =>
      spyOn(WalletCrypto.AES, 'decrypt').and.callFake((payload, key, iv, options) => {
        let payloadComponents = payload.split('|');
        let cryptoComponents = payloadComponents[1].replace('encrypted-with-', '').split('+');
    ...
    describe('decryptBufferWithKey()', () =>
      it('should decrypt when given an IV, payload and key', () => {
        let res = WalletCrypto.decryptBufferWithKey(
    ...
    describe('decryptDataWithKey()', () =>
      it('should decode Base64, extract IV and call decryptBufferWithKey()', () => {
        ...
        let key = 'c9a429594db2ae5b3df33aa651825073114dc59df230fbf74aeccab2cd99bd59';

        WalletCrypto.decryptDataWithKey(
  ...
  describe('decryptDataWithPassword()', () => // decrypt()
    it('should decode Base64, extract IV, stretch pwd and call decryptBufferWithKey()', () => {
      let data = 'zKHay4ml1+lRidNioY/Da7R5E6ERiwU7bDYsSS2UUH0=';
      ...
      let res = WalletCrypto.decrypt(data, '1234', 10);
  ...
  describe('pairing code', () =>
    it('should decrypt', () => {
      let payload = 'qf7x+dZBbUQnzgbL6SJfI8w/nracmNe4YiGIww+J40V42u4T8SgyjfJdIqPsKkQC7m0UMpQ4OpvkBl2H4NIolJizO/N0zauRnkaSeqK32rY=';
      let encryptionPhrase = 'efd93e61900251ab6cd19a4f14e5a2229866e150b04f40fe7912f392c24b5a7d';
      let guid = '6f209355-9e4b-4708-9d64-272d76bb8062';

      let decrypted = WalletCrypto.decrypt(payload, encryptionPhrase, 10);
      let components = decrypted.split('|');
  ...
  describe('decryptWalletSync()', () => {
    describe('wallet v3', () =>
      walletData.v3.forEach(({guid, enc, password}) => it(`should decrypt ${guid}`, () => {
        let dec = WalletCrypto.decryptWalletSync(enc, password);
        expect(dec.guid).toEqual(guid);
      })
      )
    );

    describe('legacy wallet v2', () =>
      walletData.v2.forEach(({guid, enc, password}) => it(`should decrypt ${guid}`, () => {
        let dec = WalletCrypto.decryptWalletSync(enc, password);
        expect(dec.guid).toEqual(guid);
      })
      )
  ...
  describe('encryptWallet()', () => {
    ...
    it('should encrypt a v3 wallet', () => {
      ...
      let enc = WalletCrypto.encryptWallet(JSON.stringify(v3.data), v3.password, v3.iterations, 3);
  ...
  describe('aes-256', () => {
    ...
    ['cbc', 'ofb', 'ecb'].forEach(mode =>
      ...
      describe(`${mode}`, () => {
        let key = new Buffer(vectors[mode].key, 'hex');
        ...
        return vectors[mode].tests.forEach(caseData => {
          let enc;

          let iv = caseData.iv ? new Buffer(caseData.iv, 'hex') : null;
          let testvector = new Buffer(caseData.testvector, 'hex');
          let ciphertext = new Buffer(caseData.ciphertext, 'hex');
          ...
          it(`should decrypt ${caseData.testvector}`, () => {
            let dec = WalletCrypto.AES.decrypt(enc, key, iv, opts);
    ...
    it('should use CBC if no mode is given', () => {
      let key = new Buffer(vectors['cbc'].key, 'hex');
      ...
      vectors['cbc'].tests.forEach(caseData => {
        ...
        let enc = WalletCrypto.AES.encrypt(testvector, key, iv, opts);
        expect(enc.compare(ciphertext)).toEqual(0);
  ...
  describe('padding', () => {
    ...
    let { pad } = WalletCrypto;
    let input = new Buffer(10).fill(0xff);

    describe('NoPadding', () => {
      it('should not add bytes when padding', () => {
        let output = pad.NoPadding.pad(input, BLOCK_SIZE_BYTES);
        expect(output.compare(input)).toEqual(0);
      });

      it('should not remove bytes when unpadding', () => {
        let output = pad.NoPadding.unpad(input);
        expect(output.compare(input)).toEqual(0);
      });
    });
    ...
    describe('ZeroPadding', () => {
      it('should fill the remaining block space with 0x00 bytes', () => {
        let output = pad.ZeroPadding.pad(input, BLOCK_SIZE_BYTES);
      ...
      it('should remove all trailing 0x00 bytes when unpadding', () => {
        let padded = Buffer.concat([ input, new Buffer(6).fill(0x00) ]);
        let output = pad.ZeroPadding.unpad(padded);
        expect(output.length).toEqual(10);
      });

      it('should unpad a ZeroPadding padded buffer', () => {
        let output = pad.ZeroPadding.unpad(pad.ZeroPadding.pad(input, BLOCK_SIZE_BYTES));
        expect(output.compare(input)).toEqual(0);
      });
    });
    ...
    describe('Iso10126', () => {
      it('should set the last byte to the padding length', () => {
        let output = pad.Iso10126.pad(input, BLOCK_SIZE_BYTES);
      ...
      it('should unpad based on the last byte', () => {
        let padded = new Buffer(BLOCK_SIZE_BYTES);
        padded[padded.length - 1] = 0x07;
        let output = pad.Iso10126.unpad(padded);
        expect(output.length).toEqual(9);
      });

      it('should unpad an Iso10126 padded buffer', () => {
        let output = pad.Iso97971.unpad(pad.Iso97971.pad(input, BLOCK_SIZE_BYTES));
        expect(output.compare(input)).toEqual(0);
      });
    });
    ...
    describe('Iso97971', () => {
      it('should set the first padding byte to 0x80', () => {
        let output = pad.Iso97971.pad(input, BLOCK_SIZE_BYTES);
  ...
  describe('cipherFunction', () => {
    it('should not modify the message is all parameters are falsy', () => expect(WalletCrypto.cipherFunction()('toto')).toEqual('toto'));
