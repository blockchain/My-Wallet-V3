...
describe "BIP38", ->
  ...
  describe "parseBIP38toECPair()", ->
    beforeEach ->
      ...
      spyOn(Bitcoin, "ECPair").and.callFake((d, compressed) ->
        cacheKey = "Bitcoin.ECPair " + d.toBuffer().toString('hex') + " " + compressed
        pubKey = undefined
        if hex = localStorage.getItem(cacheKey)
          pubKey  = Bitcoin.ECPair.fromHex(hex)

        else
          Q = ECPair.curve.G.multiply(d)
          pubKey  = new Bitcoin.ECPair(Q, compressed)
...

        {
          d: d
          pub: pubKey
          toWIF: Bitcoin.originalECPair.prototype.toWIF
        }
      )
    ...
    it "when called with correct password should fire success with the right params", ->
      ...
      # Not needed:
      # k.Q._zInv = k.Q.z.modInverse k.Q.curve.p unless k.Q._zInv?
